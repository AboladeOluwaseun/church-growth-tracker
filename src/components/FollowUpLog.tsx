...
export default function FollowUpLog({ firstTimerId, initialLogs = [] }: { firstTimerId: string, initialLogs?: FollowUp[] }) {
  const [followUps, setFollowUps] = useState<FollowUp[]>(initialLogs);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/followups', {
        ...
        body: JSON.stringify({ firstTimerId, notes: newNote, type }),
      });
      ...
      <form onSubmit={handleSubmit} className="relative group space-y-4">
        <div className="flex gap-2">
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${type === 'NOTE' ? 'bg-primary text-primary-foreground' : 'bg-secondary text-muted-foreground hover:text-foreground'}`}
          >
            <FileText size={14} />
            Visit Note
          </button>
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${type === 'CALL' ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/20' : 'bg-secondary text-muted-foreground hover:text-foreground'}`}
          >
            <Phone size={14} />
            Phone Call
          </button>
        ...
        <div className="relative">
          <textarea
            placeholder={type === 'CALL' ? "What did you discuss during the call?" : "Log a new visit note..."}
          ...
          <AnimatePresence initial={false}>
            {followUps.map((fu) => (
              <motion.div
                ...
                className={`glass-card p-5 rounded-2xl border-border bg-card/40 relative group ${fu.type === 'CALL' ? 'border-l-4 border-indigo-500/50' : ''}`}
              >
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-2">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-bold ${fu.type === 'CALL' ? 'bg-indigo-500/10 text-indigo-500' : 'bg-primary/10 text-primary'}`}>
                      {fu.type === 'CALL' ? <Phone size={10} /> : (fu.user?.name?.[0] || 'U')}
                    </div>
                    <div>
                      <span className="text-xs font-bold">{fu.user?.name || 'Unknown User'}</span>
