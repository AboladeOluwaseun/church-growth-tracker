...
export async function submitFirstTimer(formData: FormData) {
  ...
  const data: Omit<FirstTimer, 'id'> & { assignedToId?: string } = {
    ...
    dob: formData.get('dobMonth') && formData.get('dobDay') 
      ? `${formData.get('dobMonth')}-${formData.get('dobDay')}` 
      : null,
    preferredVisitTime: formData.get('preferredVisitTime') as string,
    isBornAgain: formData.get('isBornAgain') as string,
    isHolyGhostBaptized: formData.get('isHolyGhostBaptized') as string,
    discoverySource: formData.get('discoverySource') as string,
    serviceComment: formData.get('serviceComment') as string,
